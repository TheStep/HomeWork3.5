...
public class Race {
    ...
    private final LinkedList<Car> participants;
    ...
    public Race(Stage[] stages, int participantQuantity) {
        ...
        this.participants = new LinkedList<>(Arrays.asList(cars));
        this.signals = new HashMap<>();
        this.winner = new AtomicReference<>();

        signals.put(SignalType.START, new Signal("ВАЖНОЕ ОБЪЯВЛЕНИЕ >>> Гонка началась!!!", participantQuantity));
        signals.put(SignalType.FINISH, new Signal("ВАЖНОЕ ОБЪЯВЛЕНИЕ >>> Гонка закончилась!!!", participantQuantity));
    }
    ...
    public CyclicBarrier getStarted() {
        return signals.get(SignalType.START).getBarrier();
    }

    public CyclicBarrier getFinished() {
